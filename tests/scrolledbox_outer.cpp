...
void onApplicationReady(Application *self) {
    ...
    assert(self->main_widget->children[0]->children[0]->rect == Rect<float>(0, 0, 300, 300));
    assert(self->main_widget->children[0]->children[1]->rect == Rect<float>(300, 0, 300, 300));
    assert(self->main_widget->children[1]->children[0]->rect == Rect<float>(0, 300, 300, 300));
    assert(self->main_widget->children[1]->children[1]->rect == Rect<float>(300, 300, 300, 300));
}

int main() { 
    Application *app = new Application("ScrolledBox Outer Test", Size<int>(500, 500));
        app->ready_callback = onApplicationReady;

        Box *top = (new Box(Align::Horizontal))->set_background(Color());
            ScrolledBox *av_fn = (new ScrolledBox(Align::Vertical, Size<float>(300, 300)))->set_background(Color(0.0));
            ...
            ScrolledBox *av_fb = (new ScrolledBox(Align::Vertical, Size<float>(300, 300)))->set_background(Color(0.0));
        ...
        Box *bottom = (new Box(Align::Horizontal))->set_background(Color());
            ScrolledBox *av_fh = (new ScrolledBox(Align::Vertical, Size<float>(300, 300)))->set_background(Color(0.0));
            ...
            ScrolledBox *av_fv = (new ScrolledBox(Align::Vertical, Size<float>(300, 300)))->set_background(Color(0.0));
