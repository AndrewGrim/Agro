...
TextEdit::TextEdit(String text, String placeholder, i32 min_length) : Widget() {
    setText(text);
...
Size TextEdit::sizeHint(DrawingContext &dc) {
    if (m_text_changed) {
        m_virtual_size = dc.measureText(font(), text().slice(), m_tab_width);
...
i32 TextEdit::minLength() {
    return m_min_length;
}

TextEdit* TextEdit::setMinLength(i32 length) {
...
TextEdit* TextEdit::deleteAt(u64 index, bool skip) {
    if (index < text().size()) {
        if (!skip) {
            // TODO the string ctor is wrong here i think
            // essentially before we were just adding one character as the delete history text
            m_history.append(HistoryItem(HistoryItem::Action::Delete, String(m_text.data()[index]), m_selection));
        }
...
TextEdit* TextEdit::setPlaceholderText(String text) {
    m_placeholder_text = String(text);
    update();
